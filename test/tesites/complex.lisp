...
(defun slot-value* (instance slot-name)
  (bind ((class (class-of instance))
         ...
         (value
          (flet ((non-time-dependent-value (sort-function filter-function)
                   (if association-slot-p
                       ...
                       (aif (find-if filter-function (funcall sort-function matching-history-entries :ascending #f))
                            (he-value it)
                            default-value))))
            (cond ((not time-dependent-p)
                   (non-time-dependent-value (if temporal-p
                                                 #'sort-entries-by-t
               ...
               (value (cond ((primitive-type-p* slot-type)
                            ...
                            ((set-type-p* slot-type)
                             (bind ((instances (alexandria:shuffle (collect-if #L(typep !1 (prc::set-type-class-for slot-type)) instances))))
                               (subseq instances 0 (random (length instances)))))
