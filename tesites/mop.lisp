...
(defmethod validate-superclass ((class persistent-class) (superclass persistent-class-t))
  t)

(defmethod validate-superclass ((class persistent-class-t) (superclass persistent-class))
  t)

(defmethod direct-slot-definition-class ((class persistent-class-t)
                                         &key instance persistent association temporal time-dependent &allow-other-keys)
  (cond (instance
         (class-of instance))
        ((and persistent
              (not association)
              (or temporal time-dependent))
         (find-class 'persistent-direct-slot-definition-t))
        (t
         (call-next-method))))

(defmethod direct-slot-definition-class ((class persistent-association-t) &key temporal time-dependent &allow-other-keys)
  (if (or temporal time-dependent)
...
(defmethod effective-slot-definition-class ((class persistent-association-t) &key temporal time-dependent &allow-other-keys)
  (if (or temporal time-dependent)
...
(defmethod compute-persistent-effective-slot-definition-option ((class persistent-class)
                                                                (direct-slot persistent-association-end-direct-slot-definition-t)
                                                                slot-option-name
                                                                direct-slot-definitions)
  (if (member slot-option-name '(temporal time-dependent))
      (some #L(slot-initarg-and-value !1 slot-option-name) direct-slot-definitions)
